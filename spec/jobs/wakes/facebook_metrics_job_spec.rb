...
RSpec.describe Wakes::FacebookMetricsJob, :type => :job do
  ...
  it 'uses FacebookCountUpdaterService to update the pageview count on the location' do
    ...
    expect(Wakes::FacebookCountUpdaterService).to receive(:new).with(locations).and_return(service_double)
  ...
  it 'queues up the job as expected' do
    expect { described_class.perform_later(locations) }
      .to have_enqueued_job(Wakes::FacebookMetricsJob).on_queue('wakes_metrics')
  end
end
